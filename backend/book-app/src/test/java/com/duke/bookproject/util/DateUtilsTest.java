...
package com.duke.bookproject.util;
...
class DateUtilsTest {
  ...
  static void setup() {
    ...
    Mockito.when(DateUtils.TimeUtils.currentDate()).thenReturn(MOCK_CURRENT_DATE);
  }
  ...
  void correctlyCheckIfDateIsInCurrentYear() {
    ...
    assertSoftly(
        softly -> {
          ...
          softly
              .assertThat(DateUtils.dateIsInCurrentYear(MOCK_CURRENT_DATE.minusYears(1)))
              .isFalse();
          softly
              .assertThat(DateUtils.dateIsInCurrentYear(MOCK_CURRENT_DATE.plusYears(1)))
              .isFalse();
        });
  }
